...
#pragma omp parallel for num_threads(4)
    for (i = 0; i < 50; i++) /* 50 experiments50 experiments */
    {
        ...
        int A = 24;
        #pragma omp parallel for schedule(static,4) private(j,seed)shared(A,M1,N)
        for(j = 0; j < N; j++){/* 用N个数据填充初始数组 Fill the initial data array with size N */
            M1[j] = 1 + rand_r(&seed)%A;
        }
        #pragma omp parallel for schedule(static,4) private(j,seed)shared(A,M2,N)
        for(j = 0; j < N/2; j++){
        ...
        #pragma omp parallel for schedule(static,4) private(j)shared(M1,N)
        for(j = 0; j < N; j++){
        ...
        M2_temp[0] = 0;
        #pragma omp parallel for schedule(static,4) private(j)shared(M2_temp,M2,N)
        for(j = 1; j < N/2+1; j++){
            M2_temp[j] = M2[j-1];
        }
        #pragma omp parallel for schedule(static,4) private(j)shared(M2_temp,M2,N)
        ...
        #pragma omp parallel for schedule(static,4) private(j)shared(M1,M2,N)
        for(j = 0; j < N/2; j++){
            M2[j] = pow(M1[j],M2[j]);
        }
        ...
        double min = A*10;
        #pragma omp parallel for schedule(static,10) private(j)shared(min,M2,N)
        ...
        double sum = 0;
        #pragma omp parallel for schedule(static,10) private(j)shared(M2,N,min)reduction(+:sum)
        for(j = 0; j < N/2; j++){
